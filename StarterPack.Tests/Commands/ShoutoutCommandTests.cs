...
public class ShoutoutCommandTests
{
    ...
    [Fact]
    public void Name_IsShoutout()
    {
        Assert.Equal("shoutout", new ShoutoutCommand().Name);
    }

    [Fact]
    public async Task Execute_ReturnsSuccess()
    {
        var command = new ShoutoutCommand();

        var result = await command.ExecuteAsync(ContextFor("streamer", "pixelfreaki"));
