...
Usage:
    python tools/generate_import.py            # individual files (default)
...
# Registry populated by _write_export; used to build grouped files afterwards.
_EXPORT_REGISTRY: list[dict] = []

# Set by main() before generation begins; controls whether individual files are written.
_WRITE_INDIVIDUAL: bool = True
...
def make_get_variable(parent_id=None, index=0):
    return {
        "url": "https://decapi.me/twitch/uptime/%broadcastUserName%",
        "variableName": "uptime",
...
public class CPHInline
{
    ...
    public bool Execute()
    {
        ...
        if (string.IsNullOrEmpty(uptime) || uptime.Contains("is offline") || uptime.Contains("offline"))
        ...
        {
            string uptimeClean = uptime.Replace(",", "");
            CPH.TwitchAnnounce(broadcastUser + " has been live for " + uptimeClean, false, announcementColor);
        }
        return true;
    }
}"""
...
{{
    ...
    {{
        ...
        {{
            CPH.LogInfo("[points] error=" + ex.Message);
...
{{
    ...
    {{
        ...
        {{
            CPH.LogInfo("[top] error=" + ex.Message);
            CPH.SetArgument("topResult", {na_lit});
        }}
        return true;
    }}
...
def main():
    parser = argparse.ArgumentParser(description="Generate Streamer.bot import files.")
    mode = parser.add_mutually_exclusive_group()
    mode.add_argument("--group", action="store_true",
                      help="Write one combined file per action group instead of individual files.")
    mode.add_argument("--all", dest="all_modes", action="store_true",
                      help="Write both individual files and grouped files.")
    args = parser.parse_args()

    write_individual = not args.group   # default and --all
    write_grouped    = args.group or args.all_modes

    global _WRITE_INDIVIDUAL
    _WRITE_INDIVIDUAL = write_individual
    ...
    # \u2500\u2500 notifications \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500
    print()
    print("[notifications] generating 38 notification actions...")
    build_notifications(out_dir, locale_data, commands_config, queues_config)

    # ── windows notifications ────────────────────────────────────────────────────
    print()
    print("[windows_notifications] generating...")
    build_windows_notifications(out_dir, commands_config, queues_config)

    if write_grouped:
        print()
        print("[group] writing grouped import files...")
        _write_group_exports(out_dir)
...
def build_notifications(out_dir, locale_data, commands_config, queues_config):
    ...
    # ── Twitch Notification • New Follow (trigger 101) ────────────────────────
    _write("notif_twitch_new_follow", _build_notification_action(
        "Twitch Notification \u2022 New Follow", group, fun_q["id"],
        [_make_trigger(101)],
        [_label("Code"), _twitch_msg(tw["newFollow"])],
    ))

    # ── Twitch Notification • Cheer (trigger 102) ─────────────────────────────
    _write("notif_twitch_cheer", _build_notification_action(
        "Twitch Notification \u2022 Cheer", group, fun_q["id"],
        [_make_trigger(102, min=-1, max=-1)],
        [_label("Code"), _twitch_msg(tw["cheer"])],
    ))
...
_WINDOWS_NOTIFICATIONS_CS = """\
using System;
using System.Globalization;

public class CPHInline
{
\tstring TwitchIcon = "https://i.imgur.com/wTA8MlX.png";
\tstring KickIcon = "https://i.imgur.com/q0Dv9JZ.png";
\tstring YouTubeIcon = "https://i.imgur.com/mJSAzC1.png";
...
\t\t\t\t\tCPH.ShowToastNotification(toastID, "Ad Break", $"{adLength} Seconds", " ", TwitchIcon);
\t\t\t\t}
\t\t\t\tbreak;
\t\t\tcase "TwitchFollow":
\t\t\t\t{
...
\t\t\t\t\tCPH.ShowToastNotification(toastID, $"{user}", "followed", " ", userInfo.ProfileImageUrl);
\t\t\t\t}
\t\t\t\tbreak;
\t\t\tcase "TwitchHypeTrainStart":
\t\t\t\t{
\t\t\t\t\tCPH.ShowToastNotification(toastID, "Hype Train Started!", "Choo choo!", " ", TwitchIcon);
\t\t\t\t}
\t\t\t\tbreak;
\t\t\tcase "TwitchHypeTrainLevelUp":
\t\t\t\t{
\t\t\t\t\tCPH.TryGetArg("level", out string level);
\t\t\t\t\tCPH.ShowToastNotification(toastID, "Hype Train!", $"You have reached level {level}", " ", TwitchIcon);
\t\t\t\t}
\t\t\t\tbreak;
...
\t\t\t\t\tCPH.TryGetArg("percent", out string percent);
\t\t\t\t\tCPH.TryGetArg("percentDecimal", out decimal percentDecimal);
\t\t\t\t\tstring percentLabel = $"{LoadingBar(percentDecimal)} {percent}";
\t\t\t\t\tCPH.ShowToastNotification(toastID, "Hype Train Complete!", $"You reached level {level}", percentLabel, TwitchIcon);
\t\t\t\t}
\t\t\t\tbreak;
\t\t\tcase "TwitchPollCreated":
\t\t\t\t{
\t\t\t\t\tCPH.TryGetArg("poll.Title", out string title);
\t\t\t\t\tCPH.ShowToastNotification(toastID, "Poll Started!", title, " ", TwitchIcon);
\t\t\t\t}
\t\t\t\tbreak;
...
\t\t\t\t\tCPH.TryGetArg("poll.winningChoice.totalVotes", out int winningTotal);
\t\t\t\t\tCPH.TryGetArg("poll.votes", out int total);
\t\t\t\t\tdouble perc = total == 0 ? 0.0 : CalculatePercentage(winningTotal, total);
\t\t\t\t\tCPH.ShowToastNotification(toastID, "Poll results are in!", title, $"{winningVote} ({perc}%)", TwitchIcon);
\t\t\t\t}
\t\t\t\tbreak;
\t\t\tcase "TwitchPredictionCreated":
\t\t\t\t{
\t\t\t\t\tCPH.TryGetArg("prediction.Title", out string title);
\t\t\t\t\tCPH.ShowToastNotification(toastID, "Prediction Started!", title, " ", TwitchIcon);
\t\t\t\t}
\t\t\t\tbreak;
\t\t\tcase "TwitchPredictionLocked":
\t\t\t\t{
\t\t\t\t\tCPH.TryGetArg("prediction.Title", out string title);
\t\t\t\t\tCPH.ShowToastNotification(toastID, "Prediction bets are locked in!", title, " ", TwitchIcon);
\t\t\t\t}
\t\t\t\tbreak;
\t\t\tcase "TwitchPredictionCompleted":
\t\t\t\t{
\t\t\t\t\tCPH.TryGetArg("prediction.Title", out string title);
\t\t\t\t\tCPH.TryGetArg("prediction.winningOutcome.title", out string winningOutcome);
\t\t\t\t\tCPH.TryGetArg("prediction.winningOutcome.users", out string winningUsers);
\t\t\t\t\tCPH.ShowToastNotification(toastID, "Prediction Complete!", title, $"{winningOutcome} ({winningUsers} winners)", TwitchIcon);
\t\t\t\t}
\t\t\t\tbreak;
\t\t\tcase "TwitchRaid":
\t\t\t\t{
\t\t\t\t\tCPH.TryGetArg("user", out string user);
\t\t\t\t\tCPH.TryGetArg("viewers", out string viewers);
\t\t\t\t\tTwitchUserInfoEx userInfo = CPH.TwitchGetExtendedUserInfoByLogin(user);
\t\t\t\t\tCPH.ShowToastNotification(toastID, $"{user} is raiding", $"with a party of {viewers}!", " ", userInfo.ProfileImageUrl);
\t\t\t\t}
\t\t\t\tbreak;
\t\t\tcase "TwitchCheer":
\t\t\t\t{
...
\t\t\t\t\tTwitchUserInfoEx userInfo = CPH.TwitchGetExtendedUserInfoByLogin(user);
\t\t\t\t\tstring pluralized = bits == 1 ? "" : "s";
\t\t\t\t\tCPH.ShowToastNotification(toastID, $"{user} cheered", $"{bits} bit{pluralized}", $"{messageCheermotesStripped} ", userInfo.ProfileImageUrl);
\t\t\t\t}
\t\t\t\tbreak;
\t\t\tcase "TwitchSub":
\t\t\t\t{
...
\t\t\t\t\tTwitchUserInfoEx userInfo = CPH.TwitchGetExtendedUserInfoByLogin(user);
\t\t\t\t\tif (tier == "prime")
\t\t\t\t\t\tCPH.ShowToastNotification(toastID, $"{user} subscribed", $"with {tier.ToTitleCase()}", rawInput, userInfo.ProfileImageUrl);
\t\t\t\t\telse
\t\t\t\t\t\tCPH.ShowToastNotification(toastID, $"{user} subscribed", $"at {tier.ToTitleCase()}", rawInput, userInfo.ProfileImageUrl);
\t\t\t\t}
\t\t\t\tbreak;
\t\t\tcase "TwitchReSub":
\t\t\t\t{
\t\t\t\t\tCPH.TryGetArg("user", out string user);
\t\t\t\t\tCPH.TryGetArg("tier", out string tier);
\t\t\t\t\tCPH.TryGetArg("cumulative", out string cumulative);
\t\t\t\t\tCPH.TryGetArg("rawInput", out string rawInput);
\t\t\t\t\tTwitchUserInfoEx userInfo = CPH.TwitchGetExtendedUserInfoByLogin(user);
\t\t\t\t\tif (tier == "prime")
\t\t\t\t\t\tCPH.ShowToastNotification(toastID, $"{user} resubscribed", $"with {tier.ToTitleCase()} \\u2022 {cumulative} months", rawInput, userInfo.ProfileImageUrl);
\t\t\t\t\telse
\t\t\t\t\t\tCPH.ShowToastNotification(toastID, $"{user} resubscribed", $"at {tier.ToTitleCase()} \\u2022 {cumulative} months", rawInput, userInfo.ProfileImageUrl);
\t\t\t\t}
\t\t\t\tbreak;
\t\t\tcase "TwitchGiftSub":
\t\t\t\t{
\t\t\t\t\tCPH.TryGetArg("fromGiftBomb", out bool fromGiftBomb);
\t\t\t\t\tif (fromGiftBomb) break;
\t\t\t\t\tCPH.TryGetArg("user", out string user);
\t\t\t\t\tCPH.TryGetArg("recipientUser", out string recipientUser);
\t\t\t\t\tCPH.TryGetArg("tier", out string tier);
\t\t\t\t\tCPH.TryGetArg("totalSubsGifted", out int totalSubsGifted);
\t\t\t\t\tTwitchUserInfoEx userInfo = CPH.TwitchGetExtendedUserInfoByLogin(recipientUser);
\t\t\t\t\tstring attributeLabel = totalSubsGifted > 0 ? $"They've gifted {totalSubsGifted} in total!" : " ";
\t\t\t\t\tCPH.ShowToastNotification(toastID, $"{user} gifted a {tier.ToTitleCase()} sub", $"to {recipientUser}", attributeLabel, userInfo.ProfileImageUrl);
\t\t\t\t}
\t\t\t\tbreak;
\t\t\tcase "TwitchGiftBomb":
\t\t\t\t{
...
\t\t\t\t\tCPH.TryGetArg("tier", out string tier);
\t\t\t\t\tCPH.TryGetArg("totalGifts", out int totalGifts);
\t\t\t\t\tTwitchUserInfoEx userInfo = CPH.TwitchGetExtendedUserInfoByLogin(user);
\t\t\t\t\tstring attributeLabel = totalGifts > 0 ? $"They've gifted {totalGifts} in total!" : "That's a lot of subs!";
\t\t\t\t\tCPH.ShowToastNotification(toastID, $"{user} gifted", $"{gifts} {tier.ToTitleCase()} subs", attributeLabel, userInfo.ProfileImageUrl);
\t\t\t\t}
\t\t\t\tbreak;
...
\t\t\t\t{
\t\t\t\t\tCPH.TryGetArg("user", out string user);
\t\t\t\t\tCPH.ShowToastNotification(toastID, $"{user}", "followed", " ", KickIcon);
\t\t\t\t}
\t\t\t\tbreak;
\t\t\tcase "KickSubscription":
\t\t\t\t{
\t\t\t\t\tCPH.TryGetArg("user", out string user);
\t\t\t\t\tCPH.ShowToastNotification(toastID, $"{user} subscribed", "for the first time!", " ", KickIcon);
\t\t\t\t}
\t\t\t\tbreak;
\t\t\tcase "KickResubscription":
\t\t\t\t{
\t\t\t\t\tCPH.TryGetArg("user", out string user);
\t\t\t\t\tCPH.TryGetArg("monthsSubscribed", out int monthsSubscribed);
\t\t\t\t\tCPH.ShowToastNotification(toastID, $"{user} resubscribed", $"{monthsSubscribed} months", " ", KickIcon);
\t\t\t\t}
\t\t\t\tbreak;
...
\t\t\t\t\tCPH.TryGetArg("user", out string user);
\t\t\t\t\tCPH.TryGetArg("gifts", out int gifts);
\t\t\t\t\tCPH.ShowToastNotification(toastID, $"{user} gifted", $"{gifts} subs!", " ", KickIcon);
\t\t\t\t}
\t\t\t\tbreak;
...
\t\t\t\t\t\t\t\tCPH.TryGetArg("user", out string user);
\t\t\t\t\t\t\t\tCPH.TryGetArg("viewers", out string viewers);
\t\t\t\t\t\t\t\tCPH.ShowToastNotification(toastID, $"{user} is hosting", $"with a party of {viewers}!", " ", KickIcon);
\t\t\t\t\t\t\t}
\t\t\t\t\t\t\tbreak;
\t\t\t\t\t\tcase "kickPollCreated":
\t\t\t\t\t\t\t{
\t\t\t\t\t\t\t\tCPH.TryGetArg("poll.Title", out string title);
\t\t\t\t\t\t\t\tCPH.ShowToastNotification(toastID, "Poll Started!", title, " ", KickIcon);
\t\t\t\t\t\t\t}
\t\t\t\t\t\t\tbreak;
...
\t\t\t\t\t\t\t\tCPH.TryGetArg("poll.votes", out int total);
\t\t\t\t\t\t\t\tdouble perc = total == 0 ? 0.0 : CalculatePercentage(winningTotal, total);
\t\t\t\t\t\t\t\tCPH.ShowToastNotification(toastID, "Poll results are in!", title, $"{winningVote} ({perc}%)", KickIcon);
\t\t\t\t\t\t\t}
\t\t\t\t\t\t\tbreak;
\t\t\t\t\t\tcase "kickPredictionCreated":
\t\t\t\t\t\t\t{
\t\t\t\t\t\t\t\tCPH.TryGetArg("predictionTitle", out string title);
\t\t\t\t\t\t\t\tCPH.ShowToastNotification(toastID, "Prediction Started!", title, " ", KickIcon);
\t\t\t\t\t\t\t}
\t\t\t\t\t\t\tbreak;
\t\t\t\t\t\tcase "kickPredictionLocked":
\t\t\t\t\t\t\t{
\t\t\t\t\t\t\t\tCPH.TryGetArg("predictionTitle", out string title);
\t\t\t\t\t\t\t\tCPH.ShowToastNotification(toastID, "Prediction bets are locked in!", title, " ", KickIcon);
\t\t\t\t\t\t\t}
\t\t\t\t\t\t\tbreak;
\t\t\t\t\t\tcase "kickPredictionResolved":
\t\t\t\t\t\t\t{
\t\t\t\t\t\t\t\tCPH.TryGetArg("predictionTitle", out string title);
\t\t\t\t\t\t\t\tCPH.ShowToastNotification(toastID, "Prediction Complete!", title, " ", KickIcon);
\t\t\t\t\t\t\t}
\t\t\t\t\t\t\tbreak;
\t\t\t\t\t}
\t\t\t\t}
...
\t\t\t\t\tCPH.ShowToastNotification(toastID, $"{user} sent a Super Chat", amount, message, userProfileUrl);
\t\t\t\t}
\t\t\t\tbreak;
...
\t\t\t\t\tCPH.TryGetArg("user", out string user);
\t\t\t\t\tCPH.TryGetArg("amount", out string amount);
\t\t\t\t\tCPH.TryGetArg("stickerAltText", out string stickerAltText);
\t\t\t\t\tCPH.TryGetArg("userProfileUrl", out string userProfileUrl);
\t\t\t\t\tCPH.ShowToastNotification(toastID, $"{user} sent a Super Sticker", amount, stickerAltText, userProfileUrl);
\t\t\t\t}
\t\t\t\tbreak;
...
\t\t\t\t\tCPH.TryGetArg("userProfileUrl", out string userProfileUrl);
\t\t\t\t\tCPH.ShowToastNotification(toastID, $"{user} became a member!", levelName, " ", userProfileUrl);
\t\t\t\t}
\t\t\t\tbreak;
\t\t\tcase "YouTubeGiftMembershipReceived":
\t\t\t\t{
\t\t\t\t\tCPH.TryGetArg("gifterUser", out string gifterUser);
\t\t\t\t\tCPH.TryGetArg("user", out string user);
\t\t\t\t\tCPH.TryGetArg("tier", out string tier);
\t\t\t\t\tCPH.TryGetArg("userProfileUrl", out string userProfileUrl);
\t\t\t\t\tCPH.ShowToastNotification(toastID, $"{gifterUser} gifted a membership", $"to {user}", " ", userProfileUrl);
\t\t\t\t}
\t\t\t\tbreak;
\t\t}
...
\tpublic string LoadingBar(decimal value, int barLength = 10)
\t{
\t\tint filled = (int)Math.Round(value * barLength);
\t\tstring bar = "";
\t\tfor (int i = 0; i < barLength; i++)
\t\t\tbar += i < filled ? "\\u25a0" : "\\u25a1";
\t\treturn bar;
...
def build_windows_notifications(out_dir, commands_config, queues_config):
    fun_q = queues_config["fun"]
    group = "Automation"

    triggers = [
        # Twitch
        _make_trigger(139),                                        # Ad Run
        _make_trigger(101),                                        # Follow
        _make_trigger(108),                                        # Hype Train Start
        _make_trigger(110, min=-1, max=-1),                        # Hype Train Level Up
        _make_trigger(111),                                        # Hype Train End
        _make_trigger(125),                                        # Poll Start
        _make_trigger(127),                                        # Poll Results
        _make_trigger(128),                                        # Prediction Start
        _make_trigger(132),                                        # Prediction Locked
        _make_trigger(130),                                        # Prediction Results
        _make_trigger(107, min=-1, max=-1),                        # Raid
        _make_trigger(102, min=-1, max=-1),                        # Cheer
        _make_trigger(103, tiers=16),                              # New Sub
        _make_trigger(104, tiers=16, min=-1, max=-1),              # Resub
        _make_trigger(105, tiers=16, min=-1, max=-1, subType=0, monthsGifted=15),  # Gift Sub
        _make_trigger(106, tiers=16, min=-1, max=-1, subType=0),   # Gift Bomb
        # Kick
        _make_trigger(35011),                                      # Follow
        _make_trigger(35014),                                      # Sub
        _make_trigger(35015),                                      # Resub
        _make_trigger(35017),                                      # Mass Gift
        _make_trigger(18002, name="[Kick.bot] Raid",               eventName="kickIncomingRaid"),
        _make_trigger(18002, name="[Kick.bot] Poll Created",       eventName="kickPollCreated"),
        _make_trigger(18002, name="[Kick.bot] Poll Completed",     eventName="kickPollCompleted"),
        _make_trigger(18002, name="[Kick.bot] Prediction Created", eventName="kickPredictionCreated"),
        ...
        _make_trigger(18002, name="[Kick.bot] Prediction Resolved",eventName="kickPredictionResolved"),
        # YouTube
        _make_trigger(4006, min=-1.0, max=-1.0),                   # Super Chat
        _make_trigger(4007, min=-1.0, max=-1.0),                   # Super Sticker
        _make_trigger(4008),                                       # New Member
        _make_trigger(4015),                                       # Gifted Membership
    ]

    action = _build_notification_action(
        "Automation \u2022 Windows Notifications",
        group,
        fun_q["id"],
        triggers,
        [_label("Code"), _cs_subaction(_WINDOWS_NOTIFICATIONS_CS)],
    )

    _write_export(out_dir, "windows_notifications", fun_q["id"], fun_q, action,
                  command=None, commands_config=commands_config)
    print("[windows_notifications] written")
...
def _write_export(out_dir, name, queue_id, queue_def, action, command=None, commands_config=None, timers=None):
    ...
    # Register for grouped export (always)
    _EXPORT_REGISTRY.append({
        "action":    action,
        "queue":     queue_entry,
        "command":   command,
        "timers":    timers or [],
...
def _write_group_exports(out_dir):
    """Combine all registered exports by group and write one file per group."""
    from collections import defaultdict
    groups: dict[str, list] = defaultdict(list)
    for entry in _EXPORT_REGISTRY:
    ...
    for group_name, entries in sorted(groups.items()):
        ...
        queues = []
        for e in entries:
            if e["queue"]["id"] not in seen_queues:
                seen_queues.add(e["queue"]["id"])
                queues.append(e["queue"])

        actions  = [e["action"]  for e in entries]
        commands = [e["command"] for e in entries if e["command"] is not None]
        timers   = [t for e in entries for t in e["timers"]]

        export = {
            "meta": {
                "name":          f"Streamer.bot Starter Pack — {group_name}",
                "author":        "PixelFreaki",
                "version":       "1.0.0",
                "description":   f"Generated by tools/generate_import.py ({group_name} group)",
                "autoRunAction": None,
                "minimumVersion": None,
            },
            "data": {
        ...
        safe_name = group_name.lower().replace(" ", "_").replace("/", "_").replace("\\", "_")
        txt_path = os.path.join(out_dir, f"group_{safe_name}.import.txt")
        with open(txt_path, "w", encoding="utf-8") as f:
            f.write(encode_export(export))
        print(f"[group] {group_name} ({len(actions)} actions) -> group_{safe_name}.import.txt")
